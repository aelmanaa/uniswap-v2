...
import { MINIMUM_LIQUIDITY, getCreate2Address, sortTokens, encodePrice, getAmount } from '../utils/shared';
...
describe('UniswapV2Pair', () => {
  const deployFixture = async () => {
    ...
    const ERC20 = (await ethers.getContractFactory('MockERC20')) as MockERC20__factory;
  ...
  const addLiquidity = async (token0Amount: BigNumberish, token1Amount: BigNumberish) => {
  ...
  ].map((a) => [toWei(a[0]), toWei(a[1]), toWei(a[2]), getAmount(toWei(a[0]), toWei(a[1]), toWei(a[2]))]);
  swapTestCases.forEach((swapTestCase, i) => {
    it(`getInputPrice:${i}`, async () => {
      ...
      await expect(pair.swap(0, expectedOutputAmount.add(1), owner.address, '0x')).to.be.revertedWithCustomError(
        pair,
        'InvalidK'
      );
  ...
  optimisticTestCases.forEach((optimisticTestCase, i) => {
    it(`optimistic:${i}`, async () => {
      ...
      await expect(pair.swap(outputAmount.add(1), 0, owner.address, '0x')).to.be.revertedWithCustomError(
        pair,
        'InvalidK'
      );
  ...
  it('burn', async () => {
    ...
    const totalSupplyToken0 = await token0.totalSupply();
    const totalSupplyToken1 = await token1.totalSupply();
    expect(await token0.balanceOf(owner.address)).to.eq(totalSupplyToken0.sub(MINIMUM_LIQUIDITY));
    expect(await token1.balanceOf(owner.address)).to.eq(totalSupplyToken1.sub(MINIMUM_LIQUIDITY));
  });
  ...
  it('feeTo:on', async () => {
    const { pair, factory, other, token0, token1, owner } = await loadFixture(deployFixture);
